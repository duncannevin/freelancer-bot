        ...
        apiHandler.deleteBids(quote.posted_id, err => {
          ...
          apiHandler.deletePost(quote.posted_id, err => {
            ...
            state.deleteChat(this.userState.chatId, err => {
              ...
              const replyMsg = `
                ${quote.description} - has been awarded to @${quote.username}!
                \nTo update the status of this job/product go to 'Check your awarded'. \nI am sending a message to ${quote.username} to let them know. Thank you!
              `;

              const awardedMsg = `

                @${this.msg.from} has awarded you - ${quote.description} - for ${quote.price}!
                \nTo update the status of this job/product go to 'Check your awarded'. Thank you!
              `;

              this.sendMsg(replyMsg, this.msg.from, {choices: masterStages(this.userState.direction)});
              this.sendMsg(awardedMsg, quote.username, {choices: masterStages('home')});
            });
          })
        })
      })
