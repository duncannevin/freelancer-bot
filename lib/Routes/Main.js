  ...
  */
  ...
  sendErrMessage(err, number) {
    ...
    state.deleteChat(this.userState.chatId, err => {
      ...
      `);
      reply.addResponseKeyboard(masterStages('home'));
      this.bot.send(reply, this.msg.from);
    });
  }
  ...
  sendMsg(replyMsg, tooWho, {choices, image}) {
    const reply = this.Bot.Message.text(replyMsg);

    if (image) {
      let img = image.split('/big').join('');
      img = this.Bot.Message.picture(img);
      this.bot.send(img, tooWho);
    }
  ...
  clearChat(message) {
    state.deleteChat(this.userState.chatId, (err, data) => {
      ...
      this.sendMsg(message, this.msg.from, {choices: masterStages('home')});
    });
  }
