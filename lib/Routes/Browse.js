    ...
    state.updateSubStageBrowse(this.userState.chatId, {subStage: nextUp, postShowing: post.posted_id}, (err, data) => {
      ...
      this.bot.getUserProfile(post.username)
        .then(user => {
          ...
          this.sendMsg(postMsg, ['<-- Prev', 'Next -->', 'Contact', 'Main menu'], this.msg.from);
        })
  ...
  index() {
    ...
    switch(this.msg.body) {
      ...
      case 'Next -->':
        ...
        break;
      default:
        this.sendMsg(`Sorry, I didn't catch that.`, ['<-- Prev', 'Next -->', 'Main menu'], this.msg.from);
        return;
    }
  }
